...
IOSOCCamera *g_camera;

// IOSCamera
int IOSCamera::start(DVideoCaptureCallback* callback) {
    g_camera = [[IOSOCCamera alloc] initWithHanlder:callback];
    [g_camera start];
    return 0;
}
...
DArray* IOSCamera::GetVideoDevices() {
    NSArray<AVCaptureDevice*>* all_devices = [AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo];
...
DArray* IOSCamera::GetVideoDevicesAll() {
    NSArray<AVCaptureDevice*>* all_devices = [AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo];
    DULong device_count = [all_devices count];
    DArray *pRet = DArray::CreateDArray();
    for (int i = 0; i < device_count; i++) {
        DData* pItem = DData::CreateDData(30);
        pItem->AddInt8(DEVICE_PLATFORM_KEY, DEVICE_PLATFORM_IOS);
        
        AVCaptureDevice* device = [all_devices objectAtIndex:i];

        pItem->AddInt8(VIDEO_DEVICE_POSITION_KEY, (DUInt8)device.position);
...
DVoid IOSCamera::Test() {
    DArray* pDevices = IOSCamera::GetVideoDevices();
    pDevices->Dump();
    pDevices->Release();
}
#endif
